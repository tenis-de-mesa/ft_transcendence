      ...
      <table className="w-full text-sm text-left text-gray-500 table-auto dark:text-gray-400">
                  ...
                  <th className="px-6 py-3" key={header.id} colSpan={header.colSpan}>
                    {header.isPlaceholder ? null : (
                      <div {...{
                        ...
                        }[header.column.getIsSorted() as string] ?? <FaSort size={10} />}
                      </div>
                    )}
                  </th>
                )
        ...
        <tbody>
          {table.getRowModel().rows.map(row => (
            <tr className="border-b bg-gray-50 dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600" key={row.id}>
